...
class FLOWERVLA(pl.LightningModule):
    ...
    def _setup_skill_vae_conditioner(
        ...
        if self.use_skill_vae_adaln_conv1d:
            hidden_dim = int(raw_action_history_hidden_dim or encoder_dim)
            ...
            print(
                f"Using Conv1d skill history AdaLN: [B,{skill_block_size},{action_dim}] "
    ...
    def encode_skill_history(
        ...
        history = self._normalize_skill_action_history(skill_prev_actions, batch_size, device, dtype)

        if self.use_skill_vae_adaln_conv1d:
            if self.skill_vae_conv1d_encoder is None:
                raise RuntimeError("use_skill_vae_adaln_conv1d=True이지만 skill_vae_conv1d_encoder가 초기화되지 않았습니다.")
            enc_param = next(self.skill_vae_conv1d_encoder.parameters())
            skill_embed = self.skill_vae_conv1d_encoder(history.to(device=enc_param.device, dtype=enc_param.dtype))
        ...
        if self.skill_vae is None or self.skill_vae_proj is None:
            raise RuntimeError("history_adaln_source='skill_vae'이지만 SkillVAE conditioner가 초기화되지 않았습니다.")
        ...
        skill_dtype = next(self.skill_vae.parameters()).dtype
        history = history.to(device=next(self.skill_vae.parameters()).device, dtype=skill_dtype)
