...
def main():
    ...
    parser.add_argument(
        ...
        default="ft11",
        help=(
            "Use 'ft11' for [z, rpy, gripper, force, torque] without x/y, "
            "'pose7' for 7D pose+gripper, or 'ft13'/'full' for full pose+gripper+F/T."
        ),
    )
    parser.add_argument("--use_wrist", action="store_true", help="Send wrist image to server")
    parser.add_argument(
        "--transport",
        choices=["pickle", "json"],
        default="json",
    ...
    def predict_chunk(raw_obs):
        ...
        print(
            "[CLIENT timing] "
            f"obs_preprocess={(t1 - t0) * 1000:.2f} ms, "
            f"http_server={(t2 - t1) * 1000:.2f} ms, "
            f"postprocess={(t3 - t2) * 1000:.2f} ms, "
